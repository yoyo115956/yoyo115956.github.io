...
在 :eqref:`eq_batchnorm`中，$\hat{\mu}\mathcal{B}$是样本均值，$\hat{\sigma}\mathcal{B}$是小批量$\mathcal{B}$的样本标准差。应用标准化后，生成的小批量的平均值为0和单位方差为1。
由于单位方差（与其他一些魔法数）是一个任意的选择，因此我们通常包含
*拉伸参数*（scale）

$\gamma$和*偏移参数*（shift）$\beta$，它们的形状与$\mathbf{x}$相同。请注意，$\gamma$和$\beta$是需要与其他模型参数一起学习的参数。

由于在训练过程中，中间层的变化幅度不能过于剧烈，而批量规范化将每一层主动居中，并将它们重新调整为给定的平均值和大小（通过$\hat{\mu}\mathcal{B}$和$\hat{\sigma}\mathcal{B}$）。


从形式上来看，我们计算出 :eqref:`eq_batchnorm`中的$\hat{\mu}\mathcal{B}$和

${\hat{\sigma}\mathcal{B}}$，如下所示：

$$\begin{aligned} \hat{\mu}\mathcal{B} &= \frac{1}{|\mathcal{B}|} \sum_{\mathbf{x} \in \mathcal{B}} \mathbf{x},\\
\hat{\sigma}\mathcal{B}^2 &= \frac{1}{|\mathcal{B}|} \sum_{\mathbf{x} \in \mathcal{B}} (\mathbf{x} - \hat{\mu}_{\mathcal{B}})^2 + \epsilon.\end{aligned}$$

请注意，我们在方差估计值中添加一个小常量$\epsilon > 0$，以确保我们永远不会尝试除以零，即使在经验方差估计值可能消失的情况下也是如此。估计值$\hat{\mu}\mathcal{B}$和${\hat{\sigma}\mathcal{B}}$通过使用平均值和方差的噪声（noise）估计来抵消缩放问题。
...
设全连接层的输入为x，权重参数和偏置参数分别为${W}$和${b}$，激活函数为$\phi$，批量规范化的运算符为$\mathrm{BN}$。
...
```
    ...
    loss 0.276, train acc 0.898, test acc 0.843
    79257.9 examples/sec on cuda:0


![image-20240304230146503](https://s2.loli.net/2024/03/04/Rjbitw1pUgmsyTc.png)
...
```
    ...
    (tensor([1.8777, 0.1158, 2.3529, 4.1317, 0.5287, 4.0155], device='cuda:0',
            grad_fn=<ViewBackward0>),
     tensor([ 1.6335,  0.1127,  0.3697, -2.0677,  0.4760,  3.8045], device='cuda:0',
            grad_fn=<ViewBackward0>))
...
```
    ...
    loss 0.266, train acc 0.902, test acc 0.872
    85744.3 examples/sec on cuda:0


![image-20240304230228124](https://s2.loli.net/2024/03/04/8BNyFIYTmnqaVc9.png)
